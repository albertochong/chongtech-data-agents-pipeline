#!/usr/bin/env bash
# =============================================================================
# init-workspace.sh — ChongTech Agent Data Pipeline Workspace Initializer
#
...
#
# Behavior:
#   - No-ops unless the CWD looks like a ChongTech Agent Data Pipeline-aware project
...
# place to drop local agents that override ChongTech Agent Data Pipeline's plugin agents.
...
init_agent_overrides() {
    ...
    cat > "$readme" <<'EOF'
# Local Agents — Override ChongTech Agent Data Pipeline

Agents in this directory **take precedence over ChongTech Agent Data Pipeline plugin agents**
...
## Override a ChongTech Agent Data Pipeline agent
...
${CLAUDE_PLUGIN_ROOT}/agents/<name>.md   (your local override — wins)
        ↓ if absent
${CLAUDE_PLUGIN_ROOT}/agents/<name>.md   (ChongTech Agent Data Pipeline plugin)
```
...
generate_context_hint() {
    ...
    # Write the file
    {
        ...
        echo ""
        echo "> Auto-generated by ChongTech Agent Data Pipeline on $(date +%Y-%m-%d). Do not edit manually."
        echo ""
