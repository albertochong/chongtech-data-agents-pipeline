...
1. Load `${CLAUDE_PLUGIN_ROOT}/skills/sdd-brainstorm/SKILL.md` and follow it — the skill owns the methodology: KB-first grounding, one-question-at-a-time discovery, sample collection, 2-3 approach comparison, YAGNI, incremental validation, and the quality gate.
2. Produce the BRAINSTORM artifact at `.claude/sdd/features/BRAINSTORM_{FEATURE}.md`, shaped by `${CLAUDE_PLUGIN_ROOT}/sdd/templates/BRAINSTORM_TEMPLATE.md`.
...
- Skill (methodology): `${CLAUDE_PLUGIN_ROOT}/skills/sdd-brainstorm/SKILL.md`
- Agent (executor): `${CLAUDE_PLUGIN_ROOT}/agents/workflow/brainstorm-agent.md`
