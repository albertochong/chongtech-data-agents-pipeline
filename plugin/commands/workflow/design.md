...
This is **Phase 2** of the 5-phase ChongTech Agent Data Pipeline workflow:
...
1. **Load the skill** — read `${CLAUDE_PLUGIN_ROOT}/skills/sdd-design/SKILL.md`. It owns the Phase 2 methodology: KB-first resolution, the architecture and inline-ADR steps, the agent-matched file manifest, code patterns, the testing strategy, and the quality gate.
2. **Follow it end to end** (Steps 1–7) for the given `<define-file>`.
3. **Produce the DESIGN document** at `.claude/sdd/features/DESIGN_{FEATURE_NAME}.md`, update the DEFINE status per `${CLAUDE_PLUGIN_ROOT}/sdd/architecture/WORKFLOW_CONTRACTS.yaml`, and suggest `/build` as the next step.
...
- Skill: `${CLAUDE_PLUGIN_ROOT}/skills/sdd-design/SKILL.md`
